...
const EmissionTable = ({ logs }) => {
  ...
  const getScopeColor = (scope) => {
    switch (scope) {
      case 1: return 'bg-red-900/40 text-red-400 border-red-900/50';
      case 2: return 'bg-yellow-900/40 text-yellow-400 border-yellow-900/50';
      case 3: return 'bg-blue-900/40 text-blue-400 border-blue-900/50';
      default: return 'bg-gray-900/40 text-gray-400 border-gray-900/50';
    }
  };
          ...
          {logs.map((log) => {
            ...
            return (
              <tr key={log._id} className="hover:bg-green-500/5 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-[var(--text-muted)] truncate max-w-[200px]" title={`${log.activityData?.value || ''} ${log.activityData?.unit || ''}`}>
